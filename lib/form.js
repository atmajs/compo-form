	...
	var FormDataCompo = mask.Compo({
		...
		events: {
			...
			click: function click(event) {
				...
				if (this.isNativeValidation !== true) event.preventDefault();
			}
		},
		scope: {
			notificationMsg: "",
		...
		validateNative: function validateNative() {
			var el = this.$[0];
			if (el.checkValidity == null) {
				return null;
			}if (el.checkValidity()) {
				return null;
		...
		uploadEntity: function uploadEntity() {
			...
			this.xhr = Transport.send(message).fail(function (error) {
				...
				if (_this.xRedirect) {
					_this.notify("success", "" + msg + ". Redirecting...");
